...
description: Parallelized Solidity security audit across 170 attack vectors. Trigger on "audit", "check this contract", "review for security", or /solidity-auditor. Modes - default (full repo), deep (+ adversarial reasoning), or specific filename(s).
---

# Smart Contract Security Audit

> **InfoSec-Framework adaptation** of [Pashov Audit Group Skills](https://github.com/pashov/skills).  
> Reference files live in `SolidityEVM/pashov-skills/` (not `references/`).  
> `judging.md` is named `finding-validation.md` in this repo.
...
**Turn 1 — Discover.** Print the banner, then in the same message make parallel tool calls: (a) Bash `find` for in-scope `.sol` files per mode selection, (b) Glob for `**/pashov-skills/attack-vectors/attack-vectors-1.md` and extract the `pashov-skills/` directory path (two levels up from the file). Use this resolved path as `{resolved_path}` for all subsequent references.
