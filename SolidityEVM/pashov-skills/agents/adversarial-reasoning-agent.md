...
> **Cross-Reference:** Complements the Code Path Explorer and Adversarial Reviewer roles in [Audit_Assistant_Playbook.md](../../Audit_Assistant_Playbook.md)
...
1. Read all in-scope `.sol` files, plus `finding-validation.md` and `report-formatting.md` from the reference directory provided in your prompt, in a single parallel batch. Do not use any attack vector reference files.
2. Reason freely about the code — look for logic errors, unsafe external interactions, access control gaps, economic exploits, and any other vulnerability you can construct a concrete attack path for. For each potential finding, apply the FP gate from `finding-validation.md` immediately (three checks). If any check fails → drop and move on without elaborating. Only if all three pass → trace the full attack path, apply score deductions, and format the finding.
...
## Relationship to InfoSec Framework Roles

| Pashov Adversarial Agent | InfoSec Framework Role | Key Difference |
|--------------------------|----------------------|----------------|
| Free-form reasoning, no vectors | Code Path Explorer | Explorer validates ONE hypothesis; Adversarial scans ALL code |
| Confidence scoring | Adversarial Reviewer | Reviewer checks existing findings; Adversarial generates new ones |
| Logic + economic exploits | Hypothesis Generator | Generator proposes ideas; Adversarial proves them |

When used in combination: Adversarial Agent output feeds directly into the InfoSec Hypothesis Generator and Code Path Explorer for deeper manual validation.
